...
local paths = {
    jerrygivercf = workspace:FindFirstChild("Jerrygiver").CFrame,
    rhitboxcf = workspace:FindFirstChild("RefuelHitbox").CFrame,
    fuelLevels = game:GetService("Workspace").CollisionDanger.SurfaceGui.ImageLabel.Fuel.Value
}
...
local forceEngines = false
-- ui lib activated, begin yessing!
local window = library:AddWindow("IIS GUI")
local tab = window:AddTab("Main")
tab:Show()
local folder = tab:AddFolder("Engine Functions")
local e1v = false
local e2v = false
local e3v = false
local e4v = false
...
local e6v = false
local e7v = false
folder:AddSwitch("Engine 1", function(bool)
    e1v = bool
end)
folder:AddSwitch("Engine 2", function(bool)
    e2v = bool
end)
folder:AddSwitch("Engine 3", function(bool)
    e3v = bool
end)
folder:AddSwitch("Engine 4", function(bool)
    e4v = bool
end)
folder:AddSwitch("Engine 5", function(bool)
    e5v = bool
end)
folder:AddSwitch("Engine 6", function(bool)
    e6v = bool
end)
folder:AddSwitch("Engine 7", function(bool)
    e7v = bool
end)
folder:AddLabel("Determines if engine is forced the above positions")
folder:AddSwitch("Force Engine", function(bool)
    forceEngines = bool
    if bool then
        spawn(function()
            while wait(0.1) and forceEngines == true do
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine1.Value.Value == not e1v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine11.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine2.Value.Value == not e2v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine22.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine3.Value.Value == not e3v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine33.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine4.Value.Value == not e4v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine44.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine5.Value.Value == not e5v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine55.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine6.Value.Value == not e6v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine66.ClickDetector)
                end
                if game:GetService("Workspace").Engies.SurfaceGui.Frame.Engine7.Value.Value == not e7v then
                    fireclickdetector(game:GetService("Workspace").Engies.Engine77.ClickDetector)
                end
            end
        end)
    end
end)
local Fuel = tab:AddFolder("Fuel")
local fuelLevel = 80
local fueling = false
Fuel:AddSwitch("Auto Refuel", function(bool)
    fueling = bool
end)
Fuel:AddLabel("Minimum fuel before refuel")
local fuelSlider = Fuel:AddSlider("", function(p)
    fuelLevel = p
end), {
    ["min"] = "10",
    ["max"] = 99
...
fuelSlider:Set(80)
local Thermals = window:AddTab("Thermals")
local Extra = window:AddTab("Extra")
local ws = Extra:AddSlider("Walk Speed", function(p)
    hum.WalkSpeed = p
end), {
    ["min"] = "10",
    ["max"] = 99
...
ws:Set(16)
local fuelDB = false
paths.fuelLevels:GetPropertyChangedSignal("Value"):connect(function()
    if fueling == true  and fuelDB == false then 
        fuelDB = true
        if  paths.fuelLevels.Value < fuelLevel then
            local mainFrame = hrp.CFrame
            hrp.CFrame = paths.jerrygivercf
            wait(0.1)
            pcall(function()
                hum:EquipTool(plr.Backpack.Jerrycan)
            end)
            wait(0.1)
            hrp.CFrame = paths.rhitboxcf
            wait(0.2)
